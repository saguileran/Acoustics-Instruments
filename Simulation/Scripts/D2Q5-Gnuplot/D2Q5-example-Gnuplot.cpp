...
const double ke = 0, kF = 0; //k_e =  k enviorment = 0 (absortion wall)
...
public:
  ...
  void PrintGrid(const char * NombreArchivo);
...
void LatticeBoltzmann::PrintGrid(const char * NombreArchivo){
  ...
  for(int ix=0;ix<Lx;ix++){
	...
	//export data tp gnuplot visualization
...
int main(void){
  ...
  //std::cout << "unset surface; set border 4095; set ticslevel 0; set pm3d at s; set palette gray" << std::endl;
  
  std::cout << "set size ratio 1" << std::endl;
  std::cout << "set palette defined (-1 \"red\", 0 \"white\", 1 \"blue\")" << std::endl;
  std::cout << "set cbrange[-1:1]" << std::endl;
  std::cout << "set xrange[0:400]; set yrange[0:50]; set zrange[-2:2]" << std::endl;
  ...
  for(t=0;t<tmax;t++){
    ...
    Ondas.Print(t, 20+LFx,    Ly/2, "Holes-0mm.csv");
    Ondas.Print(t, 20+LFx+10, Ly/2, "Holes-10mm.csv");
    Ondas.Print(t, 20+LFx+60, Ly/2, "Holes-60mm.csv");

    //Commands to make data animation
    //if(t%5 == 0){Ondas.PrintGrid("SimpleFluteNotHoles.csv.", t);}

    //Uncomment to use GNUPLOT animation
    Ondas.PrintGrid("Sin5-5k.dat"); //Export microphoes data, time vs pressure
    std::cout << "unset title" << std::endl;
    //std::cout << "splot 'Print-Holes-Gnuplot.dat' using 1:2:3  with points palette pointsize 3 pointtype 7 " << std::endl;
    std::cout << "splot 'Sin5-5k.dat' " << std::endl;
  }
  return 0;
}
