...
#pragma omp paralel for
  ...
  for(iy=0;iy<Ly;iy++){
	...
	if(iy==Ly-1 || iy==0){fnew[ix][iy][iz][4]=k*f[ix][iy][iz][3]; fnew[ix][iy][iz][3]=k*f[ix][iy][iz][4];}
        else{fnew[ix][iy][iz][3]=UmUtau*f[ix][iy][iz][3]+Utau*feq(rho0,Jx0,Jy0,Jz0,1);
             fnew[ix][iy][iz][4]=UmUtau*f[ix][iy][iz][4]+Utau*feq(rho0,Jx0,Jy0,Jz0,2);}

        if(iz==0){fnew[ix][iy][iz][6]=k*f[ix][iy][iz][5]; fnew[ix][iy][iz][5]=k*f[ix][iy][iz][6];}
	...
	if(iz==Lz-1){fnew[ix][iy][iz][6]=0; fnew[ix][iy][iz][5]=0;}
        else{fnew[ix][iy][iz][5]=UmUtau*f[ix][iy][iz][5]+Utau*feq(rho0,Jx0,Jy0,Jz0,5);	  
	     fnew[ix][iy][iz][6]=UmUtau*f[ix][iy][iz][6]+Utau*feq(rho0,Jx0,Jy0,Jz0,6);}
...
void LatticeBoltzmann::ImponerCampos(int t){
  ...
  lambda=5; omega=2*M_PI*C/lambda; ix=proportion;  iz=Lz-proportion; A=20; iy=Ly/2;
  
  //for(iy=0; iy<Ly; iy++){
  if(t<5){rho0=A*sin(omega*t); Jx0=Jx(ix,iy,iz,false); Jy0=Jy(ix,iy,iz,false); Jz0=Jz(ix,iy,iz,false);}
  for(i=0;i<Q;i++){
